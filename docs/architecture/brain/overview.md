...
> **依據文件**：
> - `docs/architecture/specs/2026-04-27-pawai-brain-skill-first-design.md`（Brain MVS Spec — Phase A）
> - `docs/architecture/specs/2026-04-27-pawclaw-embodied-brain-evolution.md`（**PawClaw 演進** — Phase B）
...
> - `docs/architecture/specs/2026-05-06-conversation-engine-langgraph-design.md`（Phase A.5 Conversation Engine LangGraph spec — 含 graph node I/O、tools 分層、5-phase 遷移、dependency spike 步驟）
...
### pawai_brain package 結構（5/9 後實際狀態）

```
pawai_brain/
├── pawai_brain/
│   ├── conversation_graph_node.py        # ROS2 node wrapper（含 _on_reset_context 5/9 加）
│   ├── graph.py                          # LangGraph 組裝（5/9 加 mode_classifier 在 input → safety_gate 之間）
│   ├── state.py                          # ConversationState TypedDict（5/9 加 mode field）
│   ├── nodes/                            # 11 個 graph 節點
│   │   ├── safety_gate.py
│   │   ├── world_state_builder.py        # 5/9 加 current_speaker 注入（face state）
...
│   │   ├── output_builder.py
│   │   ├── trace_emitter.py
│   │   └── mode_classifier.py            # 5/9 加：5-mode regex（safety/identity/capability_question/action_request/chat）
│   └── personas/v1/                      # 5/9 加（OpenClaw-lite L7 Workspace Files）
│       ├── IDENTITY.md                   # 「你是誰」（70/20/10 小狗童心守護者）— 永遠載入
│       ├── STYLE.md                      # 怎麼說話（DO/AVOID、長度規則、對話記憶、環境）— 永遠載入
│       ├── CAPABILITIES.md               # 「你會什麼」— LAZY inject（只在 capability_question/action_request mode）
│       ├── EXAMPLES.md                   # 21+ few-shot（含 5/9 補的 identity 8 + wiggle/stretch 9 + 負例 3）— 永遠載入
│       └── OUTPUT.md                     # JSON schema + audio tag list — 永遠載入
└── launch/pawai_conversation_graph.launch.py   # 5/9：default llm_persona_file 指向 share/pawai_brain/personas/v1
```

**5/9 OpenClaw-lite persona 設計**（解 issue 2 LLM 死板真兇）：原本單檔 `tools/llm_eval/persona.txt` 把「你是誰 / 你會什麼 / few-shot / output schema」全混一起，每輪 user message 還灌 `[能力]` 17-skill JSON，LLM 被 anchor 到「工具列表」人格 → 介紹一下 = 70 字功能列表。

修法：拆 5 檔，base prompt 永遠載 4 檔（IDENTITY+STYLE+OUTPUT+EXAMPLES），CAPABILITIES.md **lazy inject**（只在 mode classifier 識別出 capability_question / action_request 才注入 user message）。chat / identity / safety mode user message 不再含 capability JSON。temperature 0.2 → 0.8（OpenClaw chat 0.7-1.0 sweet spot）。

完整設計：`docs/architecture/specs/2026-05-09-interaction-quality-improvements-design.md` P1-4。
...
| **State publisher** | `/state/pawai_brain` (TRANSIENT_LOCAL, 2 Hz)，含 mode / active_plan / safety_flags / cooldowns / last_plans ring buffer + `attention.{state,since_ts}` (5/9 加) | `brain_node.py:_publish_brain_state` |
| **Attention Policy 4-state**（5/9 加）| `AttentionMachine` 純 Python state machine：`IDLE` → `NOTICED` (face 0.5s stable) → `ENGAGED` (distance ≤ 1.6m AND dwell ≥ 1.5s) → `INTERACTING` (active plan / speech intent)；emit gate per skill：greet ENGAGED-only / object_remark + active_plan/pending/tts not / stranger_alert + active_skill guards；race-safe `_attention_state_snapshot()` 取代裸讀。10Hz `_tick_attention` timer。 | `interaction_executive/attention_machine.py` + `brain_node.py` |
| **Idle MVP（P3-1a）**（5/9 加，default OFF）| 12 個 audio-tagged 短語、6 道 gate（idle_enabled / threshold / cooldown / no active_plan / no pending_confirm / not tts_playing / attention IDLE）、`idle_max_per_hour` 真 enforcement（`idle_emit_history` deque + 3600s window）、`_touch_user_interaction()` 在 4 個 callback 重置時鐘、復用 10Hz `_tick_attention` timer。Roy 訴求「閒置 10 分鐘」對應 `idle_threshold_s=600` home profile。 | `brain_node.py:_maybe_emit_idle` |
...
| 主題 | 文件 |
|---|---|
| **本文件**（系統整合總覽） | `docs/architecture/brain/overview.md` |
| **Phase A**：Brain MVS 設計 spec | `docs/architecture/specs/2026-04-27-pawai-brain-skill-first-design.md` |
| **Phase A**：Brain MVS 實作 plan（34 tasks） | `docs/archive/pawai-brain-legacy/plans/2026-04-27-pawai-brain-skill-first.md` |
| **Phase A.5**：Conversation Engine LangGraph 演進 spec | `docs/architecture/specs/2026-05-06-conversation-engine-langgraph-design.md` |
| **canonical Capability Claim Matrix**（能力分級真相源） | `docs/mission/2026-06-18-capability-claim-matrix.md`（判決來源：6/05 audit §B） |
| **能力門檻 / 怎麼量** | `docs/architecture/specs/2026-06-18-capability-baseline-spec.md` |
| **最新實測 trusted snapshot** | `docs/runbook/baseline-evidence/2026-06-04-hitl/` |
| **Phase B**：PawClaw Embodied Brain V1 演進 spec | `docs/architecture/specs/2026-04-27-pawclaw-embodied-brain-evolution.md` |
...
| PawAI Studio 設計 | `docs/architecture/studio/README.md` |
...
詳細設計請見 `docs/architecture/specs/2026-04-27-pawclaw-embodied-brain-evolution.md`。
