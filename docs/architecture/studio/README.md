...
| `POST /api/text_input` | 瀏覽器→ROS2 | 文字輸入 → /brain/text_input（live）/ Gemini call（mock + MOCK_OPENROUTER）。**5/9 night 加 return `text` 欄位**：gateway 套 s2twp 後回傳，frontend update user bubble 顯示繁中（issue 6 frontend 補）| ✅ | ✅ |
...
| `WS /ws/speech` | 瀏覽器→ROS2 | 錄音 → ASR（5/9 加 OpenCC s2twp 繁化）→ intent（5MB cap） | ✅ | ✅ |
...
ChatPanel header 加「新對話」按鈕（`handleNewConversation`），點擊：
1. `window.confirm("將清除目前所有對話記憶，包括其他開啟的 Studio 視窗。確定？")`
2. `fetch('/api/reset', {method: 'POST'})` → backend 清 brain memory + seen_sessions
3. `setMessages([])` 清前端 messages
4. `useStateStore.setState({ ttsMessages: [] })` 清 ring buffer
5. `lastSeenTtsIdRef.current = null`

**Dev-only F5 hybrid auto-detect**（demo 預設關）：
- env flag `NEXT_PUBLIC_AUTO_RESET_ON_REFRESH=false`（default）
- 設 `=true` 開：layout 註冊 `beforeunload` 寫 `sessionStorage.paw_refresh_at`，`use-websocket.ts onopen` 檢查 5s 內 → 自動 POST `/api/reset`
- demo 期建議靠手動按鈕，因 brain memory 全局單例，F5 一台會清全 brain

### 5/9 night 補充：ChatPanel stick-to-bottom（issue 9）

`chat-panel.tsx` 原本 `useEffect(() => bottomRef.scrollIntoView(), [messages, isThinking])` 無條件每次 messages 變動就把畫面拉回底，user scroll up 看舊訊息會被新訊息打斷。

修法（commit `87e2d5d`）：仿 Slack/Discord stick-to-bottom 模式
- 加 `scrollContainerRef` + `shouldAutoScrollRef`
- 滾動容器 `onScroll` handler：`scrollHeight - scrollTop - clientHeight < 30` 視為「附著底部」
- useEffect 改條件 `if (shouldAutoScrollRef.current) bottomRef.scrollIntoView()`

驗證：跟狗連講 10 句、向上 scroll 看第 1 句 → 新訊息進來不會把視角拉回底；scroll 回最底 → 新訊息進來自動跟。

### 5/10 night 補充：Composer absolute-bottom layout 重構（Spec 6 P0）

5/9 stick-to-bottom fix 之後實測雖然 scroll 行為對了，但 composer（輸入框）會被訊息推離 viewport 底部、發長訊息有空白 catch-up window。一連串嘗試（`flex h-full flex-col overflow-hidden` + `min-h-0` + `shrink-0` + `behavior: "auto"` + `isAutoScrollingRef` lock 程式 scroll 事件）只解了部分症狀。

根因：composer 在 message flex 流中、依賴 flex sizing 維持位置；textarea auto-grow / skill strip toggle / 訊息突增任一觸發都會微跳。

修法（commit `fdd5c93`）：抽 Composer + ChatGPT-like absolute layout
- 新檔 `components/chat/composer.tsx` — 純 view，props 餵 state/handlers，`useAudioRecorder` owner 仍在 ChatPanel
- ChatPanel conversation view root：`relative h-full overflow-hidden`
- 三層結構：
  - header（normal flow，`headerRef` 量高）
  - scroll area（`absolute inset-x-0` + 動態 `style={{ top: headerH, bottom: composerBarH }}`）
  - composer bar（`absolute inset-x-0 bottom-0 z-20` 包 skill strip + Composer）
- `ResizeObserver` 觀察 header + composer bar，寫入 useState；初始值 44/96 避免首幀 top:0 蓋 header
- z-index 約定：composer z-20 < DevButton z-30 < Sheet z-40/50（dev-button.tsx:22 driven）
...
### 5/9 night 補充：OpenCC API name silent fail 修正

`text_normalization.py`（gateway + speech_processor 兩處）原本 `OpenCC("s2twp.json")`，`opencc-python-reimplemented` 會自動 append `.json` → 嘗試載入 `s2twp.json.json` → FileNotFoundError → 落到 except → silent fallback 原文。

修法：改 `OpenCC("s2twp")`。從 PR #52 起三條 ASR / 文字 簡→繁入口都 silent fail，這次（commit `756aeb0`）才被抓到。

### 5/9 補充：ChatPanel 全 utterance 顯示（issue 5）— 已 5/8 smoke 確認

state-store `ttsMessages: TtsMessage[]` 200-entry ring buffer + dedup by id + rate-limit（spontaneous 5s/source；bypass `chat_reply` / `skill_say` / `say_canned`）。ChatPanel 監聽 `ttsMessages` array append（不再用 `pendingRequestIdRef` gate display）。CSS 三色：`source=skill_say` 綠 / `say_canned` 橙 / `chat_reply or pending` 一般灰 / 無 source spontaneous 淡灰 + ⏰ icon。

source 欄位由 `interaction_executive/skill_contract.py:_resolve_say_source()` 在 `build_plan` 注入 SAY step args，IE-node `_dispatch_step` 帶進 `/tts` JSON envelope，gateway `_parse_tts_payload` 解析後 `build_tts_event` 帶進 ws/events broadcast。
...
詳細 Schema + status enum：`docs/contracts/interaction_contract.md` v2.7（Phase 0.5 章節）+ spec `docs/architecture/specs/2026-05-06-conversation-engine-langgraph-design.md` §4。
