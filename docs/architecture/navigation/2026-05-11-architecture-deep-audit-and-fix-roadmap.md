...
> **Authority**: 本檔取代任何 reactive_stop 參數、Nav2 obstacle_layer 配置、感測器分工的舊敘述。`docs/architecture/navigation/CLAUDE.md` 與 `docs/archive/pawai-brain-legacy/plans/2026-05-11-nav-root-cause-burndown.md §4` 引用本檔。
...
`docs/architecture/navigation/CLAUDE.md` 5/1 寫過「safety_only ... clear zone shadow nav」警告，但被當成工程細節，沒升等成「demo 致命設計缺陷」。
...
| # | 修法 | 狀態 |
|:-:|---|:-:|
| B3.1 | `docs/architecture/navigation/CLAUDE.md` 把「safety_only clear zone shadow nav」升等成 architecture-critical 段落，故事化描述 + 4-mode 表格 + 釋放策略 | ✅ commit `0f5a16f` |
...
- ✅ docs/architecture/navigation/CLAUDE.md B3.1 升等 architecture-critical
...
- **取代**：`docs/architecture/navigation/CLAUDE.md` 中關於 reactive_stop 參數的舊敘述（5/1 「safety_only ... clear zone 會 shadow nav」需 B3.1 升等成 architecture-critical）
...
- **與 `docs/architecture/navigation/specs/2026-05-03-d435-rplidar-fusion-detour.md` 關係**：detour profile（`/scan_d435 + d435_scan`）已設計，但 demo 期不啟用、demo 後 B2.1 才落地
