...
>
> **與 Phase B PawClaw 對接**：Phase B (`docs/architecture/specs/2026-04-27-pawclaw-embodied-brain-evolution.md`) 已規劃 `workspace/{BODY,SKILLS,SAFETY,PLACES,DEMO_MODE}.md` 但目的是**人類可讀文件**，Phase C 才注入 LLM。本 5/9 P1-4 拆的 5 檔是 **chat path LLM prompt 用**，命名 `pawai_brain/personas/v1/{IDENTITY,STYLE,CAPABILITIES,EXAMPLES,OUTPUT}.md` — 不同 namespace、不同階段、不衝突。Phase C 合流時可由 generator 從 BODY/SKILLS 抽 → personas/CAPABILITIES。
