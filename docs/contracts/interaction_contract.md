...
| `/brain/demo_phase` | Command | 觸發式 | Studio → Brain 五幕 demo phase 切換（std_msgs/String：`all`/`s1_nav`/`s2_greet`/`s3_pose_object`/`s4_gesture`/`s5_safety`/`quiet`；legacy alias `s2_face`/`s3_object`）— plan2 Conductor，與 `ros2 param set /brain_node demo_phase` 共用 `_set_demo_phase`，phase 真正變更時清 pending_confirm + active_plan + gesture cooldown（**不清** attention）| **v2.13** active |
| `/brain/offline_mode` | Command | 觸發式 | Studio → Brain 離線模式總開關（std_msgs/Bool）— plan3 T4，與 `ros2 param set /brain_node offline_mode` 共用 `_set_offline_mode`；default False = byte-identical（只在明確 set 時改變）| **v2.14** active |
