...
- ❌ **單純居家陪伴狗**：不主打一般家庭採購、不主打單純陪伴；居家延伸場景留作平台未來能力，6/18 不驗（詳 ADR-0002 雙層敘事）
- ❌ **語音助理**：Apple / 小米早做且更穩，差異化不足
- ❌ **自由巡邏機器人**：6/18 內做不到大型場域動態繞行
- ❌ **直接物理接觸長者的互動鏡頭**：20 kg 機體 + 走路晃動 + sport mode MIN_X 0.5 m/s，任何接觸式互動都是安全紅線（詳 ADR-0001 非接觸式定位）
...
### 雙層敘事 footnote（2026-05-23 grill 增）

PawAI 對外定位採**雙層**結構：

- **平台身份**：通用居家 / 機構四足互動機器人（能力涵蓋移動、多模態感知、語音對話、安全仲裁、可擴充技能組合）
- **2026-06 demo 場景**：非接觸式機構巡檢助理（本文件主軸）

未來不同場景採購方（校園、商場、居家、導盲、救災）對平台的需求不同，由他們延伸；2026-06 PawAI 團隊只負責驗證機構巡檢這條場景。詳見 ADR-0002。
...
### 人體安全紅線（2026-05-23 grill 增）

ADR-0001 明確 PawAI 2026-06 POC 為**非接觸式機構巡檢助理**。三條落地規則：

1. PawAI 不主動靠近長者過近距離（具體閾值由 reactive_stop / depth_safety 涵蓋，本規則不訂死數字）
2. Demo 影片 / 簡報 / 對外材料**不出現長者物理接觸 PawAI 鏡頭**
3. 一切物理協助由照護人員執行，PawAI 只負責感知、提醒、回報

未來如要重啟「主動靠近 fallen 確認 / 攙扶 / 安撫」必須開新 ADR + safety case，不能私自繞過。

### 對外敘事公式（2026-05-23 grill 增）

詳見獨立文件 `docs/pawai-demo/2026-05-23-doc-and-report-guidelines.md`，收錄：
- 禁用詞清單（不講「做不到 / 比較差 / 期待不要太高」）
- 必說句清單（Before/After 範式 + 三鐵律 + 安全聲明）
- 整合挑戰寫法（3D 列印 / 降壓板 / SDK 破解 / NumPy 治理）
- Before/After 句式範例
...
| 人機接觸風險：20 kg 機體 + 移動中急停產生晃動 → demo 全程需保 ≥1m 距離 | **P0** | 詳 ADR-0001；reactive_stop 0.6m danger zone 涵蓋大部分，demo 排練再 audit 一次 |
| F7 bug：nav goal accepted 但 `/cmd_vel_nav` 不出 publisher、10s timeout（5/12 已知未解）| **demo blocker** | W1 1d 偵錯 spike（新增），W1 未鎖 → W2 plan B 先做不依賴 F7 的接線 |
...
## 7. Engagement Gate：多源觸發抽象（2026-05-23 重寫）

> **重要變更**：原 v2 spec 規劃 OpenWakeWord「嗨 PawAI」+ VAD swap。5/22 grill 後決議改用 **Studio Push-to-Talk + face/gesture 三源觸發**，**不做 wake word / 拔 VAD**。詳見 ADR-0003。

### 戰略價值

機構巡檢場景下，trigger 來源有兩類使用者：
- **照護人員主動派遣**：在 Studio 端**按 mic 按鈕**講話「PawAI，去活動區巡一下」
- **長者主動發起互動**：揮手 / 比 OK 手勢 / 走近並 face 直視 PawAI

兩類都走同一條 Engagement Gate 抽象。**陌生人路過不打招呼、不亂回應**——這是「不打擾」的設計，符合機構環境禮儀。

### Studio Push-to-Talk + 統一抽象

PawAI Studio composer 已有 mic button（`pawai-studio/frontend/components/chat/composer.tsx`，已實作 isRecording / startRecording / stopRecording / AudioVisualizer）。**真正的 spec 是 Engagement Gate 抽象層**——把 Studio PTT / OK 手勢 / face approach 統一成一個 `/event/engagement_triggered` topic：

```json
{
  "source": "studio_ptt" | "gesture_ok" | "face_approach",
  "confidence": 0.92,
  "ts": ...,
  "ttl_sec": 8.0
}
...
- `stt_intent_node` → 只在 engagement 期間跑 ASR + 發 LLM
- AMBIENT 時 ASR 完全不跑（省 cloud cost + 省 Jetson GPU）

### State machine

```
AMBIENT (idle, ~5% CPU, ASR 不跑)
  └─[studio_ptt press / gesture_ok / face_approach]→ ATTENTIVE (8s window)
        ├─[mic release (PTT) 或 speech end (face/gesture)]→ ASR → LLM → TTS → 回 AMBIENT
        └─[timeout 8s 無互動]→ 回 AMBIENT
```

**PTT 細節**：mic button 按下 → ATTENTIVE + 開始錄音、放開 → 結束錄音直接送 ASR（**跳過 VAD**，無 2-10s 飄移）。

### 對齊「為什麼是狗」

| 能力 | 狗的對應 |
|---|---|
| 照護人員 PTT 派遣 | 主人叫指令派出去 |
| OK 手勢進手勢模式 | 狗對主人手勢敏感 |
...
- ❌ OpenWakeWord「嗨 PawAI」自訓 wake word（ADR-0003 凍結；未來居家延伸場景若需要 always-on 互動再開新 ADR）
- ❌ VAD（energy / Silero 全砍，PTT 取代）
...
原 v2 L2-G 3d → 本版 ~1.5-2d（mic button → ASR direct path + engagement source enum 加 `studio_ptt` + face/gesture 訂閱發 event）。**回收 1-1.5d** 給 nav / object 主線。
...
### 兩個 deadline 性質截然不同（2026-05-23 增）

> **重要**：5/27 是**唯一影片產出**（可剪輯、可重錄）；6/18 是**實機 live demo**（Go2 帶去學校、不能剪）。兩個 milestone 風險、scope、敘事策略全不同，本節分兩段處理。

| Deadline | 性質 | Scope | 風險 |
|---|---|---|---|
| **5/27** | 錄製影片（可剪輯） | **門口監控小閉環**（熟人 + 陌生人 + 語音指令 + 安全拒絕，~3 分鐘） | 4 天 timeline；功能爆掉可剪、可重錄 |
| **6/18** | Go2 帶去學校實機 live demo | **長照交誼廳場景劇**（8 mini-event 連拍 ~3 分鐘） | live 不可剪；現場觀眾看真實 PawAI |

兩個 deadline 共用 §11 段 0 硬體 Before/After + §11 段 4 未來情境收尾。

### Demo A（5/27 影片）：門口監控小閉環

**詳見獨立 spec**：[`docs/pawai-demo/2026-05-27-mid-video-spec.md`](../../pawai-demo/2026-05-27-mid-video-spec.md)（5/23 grill + codebase audit 後 lock）
...
| # | 鏡頭 | 對應能力 | 4 天工程量 |
|---|---|---|---|
| 1 | 開場 Before/After | 旁白 + 硬體素材（段 0 共用） | Roy 在家拍 |
| 2 | 熟人辨識 + 主動打招呼 | face_identity（已 USABLE） | 0 LoC |
| 3 | 陌生人追問 + 警示 | 新陌生人 FSM（thin 版 3 輪固定問句 + LLM 判斷） | 180-220 LoC |
| 4 | 語音指令「坐下 / 舉手」精準執行 | intent_classifier 加「舉手」keyword + LLM whitelist | 20 LoC |
| 5 | 安全拒絕「翻跟斗」+ Studio trace 紅色 | intent_classifier 新 `unsafe_intent` 欄位 + brain 直接 reject path（不進 skill_dispatcher、不產生可執行 plan）+ safety TTS。LLM whitelist 完全不動。詳 [5/27 spec § 5](../../pawai-demo/2026-05-27-mid-video-spec.md) | 60-80 LoC |
| 6 | 結尾接三週方向 + 段 4 未來情境 | 旁白 | 0 LoC |

**Nice-to-have（5/27 非 blocker）**：口罩 / 帽子 遮擋 fallback（80-110 LoC）— 易被光線 / 角度 / 模型狀態搞爆，穩定就拍、不穩就剪。

**5/24-5/27 排程**（Roy own 功能、團隊接拍攝）：

- 5/24：寫陌生人 FSM + 舉手 keyword + 翻跟斗安全路徑 + 初步 smoke
- 5/25：Jetson 實測 + 拍硬體 Before/After 素材 + 修爆掉功能
- 5/26：排練 + 錄備用素材，**不再加功能**
- 5/27：拍攝 + 剪輯 + 交件

**砍項**（5/25 PM smoke 不穩時）：首砍能力 3 retry loop（保留 1 問 1 答 + 警告）/ 次砍能力 2 遮擋。**能力 4+5 不可砍**（PawAI Brain 三層架構唯一可視化證據）。

### Demo B（6/18 實機 live）：多事件串連場景劇 live runbook（2026-05-23 重寫 + 5/23 evening 修正：純 live，無剪輯/預錄）

> **重要變更**：原 v2 規劃「一條任務閉環三段呈現」。5/22 grill 後改為**呂奇傑情境 3 風格的多事件串連場景劇**——同一個長照交誼廳場景內 ~3 分鐘 live 連跑（不能剪輯）。對應呂奇傑「光是 function 能做出來就不容易，串在一起就 OK 了」+ 雅文「蘿蔔水果湯要變成湯」雙重訊號。
>
> **6/18 是 live demo，不是影片**：以下所有 mini-event 在現場 live 跑，**不能 cut、不能後製、不能 AI 生成補完**。任何「影片結構 / 預錄 / AI 生成」用法皆移除或標為「**開場輔助投影片，不屬 live demo 本體**」。
>
> ADR-0004 將於本節落地後寫入。

#### 開場輔助投影片（不屬 live demo 本體）

6/18 現場演示**開始前**，可放投影片或 5/27 中期影片片段建立可信度，**這不是 live demo 的一部分**：

- **開場 5/27 中期影片 ~3 分鐘**（觀眾看過 PawAI 核心 function：門口監控 + 語音動作 + 安全拒絕）
- 或單張投影片：硬體 Before/After 圖文（Go2 + Jetson + 降壓板 + 3D 列印外殼）— 對應呂奇傑 5/22「硬體故事必須講」訊號
- **未來情境投影片**：校園 → 導盲 → 救災 三張靜態圖（不是 AI 生成動畫，是 closing 用的單張投影片）

開場輔助 ≤ 4 分鐘。**結束後 PawAI 在現場待機，準備跑 live runbook**。

#### Live runbook：場景劇 mini-event 連跑（~3 分鐘 live，不可剪）

**場景**：機構交誼廳（拍攝替代場 = 家裡客廳布置成多人坐姿）。**演員**：Roy（照護人員兼操作 Studio）+ 鄔 / 楊 / 黃 / 陳（住民甲乙丙）。

| 時間 | mini-event | 觸發條件 | PawAI 反應 | 對應能力 | 為什麼是狗 |
|---|---|---|---|---|:---:|
| 0:30 | **e1 派遣** | Roy 在 Studio 按 mic「PawAI，去活動區巡一下」 | PawAI 啟動 + 走向 named waypoint「活動區」 | Engagement Gate (studio_ptt) + NAV executor + Named waypoint | 移動 ✅ 實體 ✅ |
| 0:50 | **e2 認人** | face_identity 在交誼廳認出住民甲（張奶奶） | TTS「張奶奶您好」 | face_identity + LLM persona | 實體 ✅ 多模態 ✅ |
| 1:10 | **e3 久坐關心** | pose classifier 偵測住民乙（王爺爺）久坐 30s | PawAI 走近（保持 ≥1m，ADR-0001）+ TTS「您坐很久了，會不會太累，我陪您一下」 | Pose state + Brain rule + safety distance | 移動 ✅ 實體 ✅ 多模態 ✅ |
| 1:30 | **e4 物品提醒** | object_perception 偵測地上水杯 + Object schema v2 算 bearing/depth | TTS「阿嬤，您的水杯掉在左前方 1.5 公尺，要記得撿起來」 | Object schema v2 + find_nearest + LLM | 多模態 ✅ |
| 1:50 | **e5 語音指令精準回應** | Roy 在 Studio PTT「PawAI，舉手」 | Skill 精準執行 hello/raise（**不是 LLM 自由發揮，是 deterministic skill**） | skill_dispatcher + Safety pass | 多模態 ✅（呂奇傑指定鏡頭）|
| 2:05 | **e6 安全機制拒絕** | Roy 在 Studio PTT「PawAI，請翻跟斗」 | Safety Layer 攔截 + TTS「這個動作不安全，我不能做」+ Studio trace 紅色 highlight | SafetyLayer + Studio trace | 安全 ✅（呂奇傑指定鏡頭、ADR-0001 唯一可視化證據）|
| 2:20 | **e7 (if-branch) 跌倒** | fallen state machine 觸發（住民丙演跌倒，用瑜珈墊預錄 cut 進去） | PawAI 走近觀察（**不接觸、不攙扶**，ADR-0001）+ Studio trace 顯示 fallen event（TTS silent，由段 3 回報講出） | fallen state machine + Studio trace + reactive_stop | 移動 ✅ |
| 2:35 | **e8 巡檢回報** | Roy 在 Studio PTT「PawAI，回報」+ PawAI 走回鏡頭前 | VLM scene describe + 多源融合 → TTS「活動區目前有 3 位住民，張奶奶在窗邊，王爺爺在沙發。有一位長者倒在地上，請工作人員立刻確認」 | VLM scene describe + face_identity + object_schema + patrol_status + TTS | 多模態 ✅ |

**Live runbook 設計**：
- mini-event 觸發順序**用 vision_perception / brain 既有觸發機制自然發生**，不是腳本硬塞時間軸
- **跌倒 mini-event（e7）必須真實 live 觸發**（住民丙演跌倒用瑜珈墊保護），**不能用預錄影片 cut**（6/18 live 不可剪）
  - 若 fallen state machine 在現場 ≥30s 無觸發 → 段 e8 巡檢回報帶過「無異常」，不強行
  - **預錄 cut 是 5/27 影片才用的手法**（5/27 spec § 6 fallback 表）
- 全程 Studio trace 在現場大螢幕角落顯示，作為「系統決策透明化」對觀眾現場展示
- 全程**不出現任何住民物理接觸 PawAI 鏡頭**（ADR-0001 紅線）

#### Live demo 收尾（投影片 + 主講人口頭）

Live runbook 跑完後，**不再有 mini-event**。主講人現場口頭 + 投影片帶完：

- 主講人總結：「**機構巡檢只是 6/18 POC 場景；PawAI 的平台能力可延伸到校園、導盲、救災。**」（雙層敘事，ADR-0002）
- 投影片放 3 張靜態圖（校園 → 導盲 → 救災，順序對齊 v2 §12 + grill Q3）
- 最後一張黑底白字：「**PawAI — Physical AI for Institutional Care**」

**這段是投影片 + 主講人，不是 PawAI 自己跑的 mini-event**。

### Demo failure fallback（2026-05-23 重寫）

每個 mini-event 必須有 fallback path，避免 demo 現場啞掉：

| mini-event / 技能 | 主路徑 | Fallback |
|---|---|---|
| e1 派遣 (NAV executor + named waypoint) | Nav2 走到 named waypoint | 原地轉身 + 語音「我無法移動，但我聽到你了」|
| e1-e7 Studio PTT 觸發 | mic button 按住錄音 | Studio 鍵盤輸入文字（chat-panel 已有）|
| e2 認人 (face_identity) | 識別張奶奶 | 「我看到一位住民」（不喊名）|
| e3 久坐關心 (pose) | pose classifier sitting + 30s timer | 跳過此 mini-event，PawAI 繼續往下走 |
| e4 物品提醒 (Object schema v2) | depth + bearing good | depth_m unavailable → 「我看到水杯但距離測不到」|
| e5 語音指令精準回應 | skill_dispatcher routing | LLM 自然語言 fallback「好的我來舉手」+ 嘗試 skill；都失敗 → TTS 道歉 |
| e6 安全機制拒絕 | SafetyLayer 攔截 + Studio trace | **不能 fallback**——拒絕是 happy path，失敗代表 ADR-0001 紅線破洞 |
| e7 跌倒 if-branch | fallen state machine 觸發（住民丙演跌倒，現場演員配合）| state machine 無觸發 → 段 e8 巡檢回報直接帶過「無異常」，不影響主敘事；**live 不能 cut**（5/27 影片才可預錄 cut，6/18 是 live demo）|
| e8 VLM 巡檢回報 | Gemini 2.5 Flash | YOLO person count → 「我偵測到 N 個人，請工作人員親自查看」|
| Brain LLM | gpt-5.4-mini | gemini-3-flash → RuleBrain rescue |
| TTS | Gemini Despina | edge-tts → Piper |

**6/18 Live Demo 演出策略**：

- 開場可先放 5/27 中期影片建立可信度（觀眾已看過 PawAI 核心 function）
- 現場 live 跑段 1-3 場景劇證明真實（5/27 影片已建立期待，live 加強）
- 即使 live 出包，5/27 影片仍是書面交付 + 觀眾已看過大概
- **不錄製 6/18 影片** — 6/18 是 live demo，5/27 是唯一影片產出

### Live 連跑可行性 Spike（W3 內驗）

Live demo ~2.5 分鐘的場景劇連跑需要：
- 五個感知 + brain + nav stack 同時跑 ~3 分鐘不出包
- mini-event 觸發順序穩定（不會跳過 / 不會重複觸發）
- Studio trace TTS chain 不卡
- 現場演員配合（住民甲乙丙、跌倒演出含瑜珈墊保護）

**W3 spike #5（live 連跑 spike）**：6/8-6/10 在 main branch 連跑段 1-3 雙原型。FAIL → mini-event 數降規模（8 → 5-6），保核心串連敘事。詳 §13 spike #5。

---

### 原任務閉環三段 (deprecated 2026-05-23，留作對照)
...
- ❌ **PawAI 單純定位成居家陪伴狗**（一般家庭非合理採購方；居家延伸列平台未來能力，6/18 不驗，詳 ADR-0002 雙層敘事）
- ❌ **PawAI 取代照護人員**（定位是輔助第一線確認，不取代專業判斷）
- ❌ **PawAI 直接物理接觸長者**（攙扶 / 碰觸 / 推拉 / 承重 / 安撫性肢體互動全擋；demo 影片不出現接觸鏡頭，詳 ADR-0001 非接觸式定位）
...
**2026-05-23 grill 後 spike 表新增條目**：

| spike | 驗收標準 | PASS → | FAIL → |
|---|---|---|---|
| **#1d F7 偵錯 spike**（W1 1d，新增） | 鎖定 F7 root cause：nav goal accepted 但 `/cmd_vel_nav` 不出 publisher、10s timeout（5/12 已知未解）| W2 順排 L2-A1 depth fusion + NAV executor 接線 | **W2 plan B**：先做 4 件不依賴 F7 的接線（NAV executor + named waypoint + schema drift 修 + nav_ready 升級），F7 retry + depth fusion 壓 W3 |
| **#4 5/27 影片必拍能力 spike**（W1 1-1.5d，新增） | 5/24-5/25 main branch 跑通 5/27 影片 4 條能力：(a) 熟人辨識 + 主動打招呼 5/5、(b) 陌生人 FSM thin 版 + LLM 判斷 3/5、(c) 「坐下 / 舉手」精準 skill 5/5、(d) 翻跟斗拒絕 + Studio trace 紅色 5/5 | 鎖進 5/27 影片拍攝（5/26-5/27） | 砍 retry loop / 砍遮擋 fallback（詳 [5/27 spec § 6](../../pawai-demo/2026-05-27-mid-video-spec.md)）|
| **#5 6/18 live 連跑可行性 spike**（W3 中，新增） | 6/8-6/10 main branch 連跑 §11 Demo B 場景劇段 1-3：(a) mini-event 觸發順序穩定、(b) Studio trace + TTS chain 不卡、(c) 五個感知 + brain + nav stack 同時跑 ~3 分鐘不出包 | 鎖進 6/18 live demo 排練 | mini-event 數降規模（8 → 5-6），保核心串連敘事 |
| **#6 PINTO_model_zoo shortlist spike**（W2 初，新增） | 5/28 後 0.5-1d 研究 4 類候選（遮擋 / 揮手 / 坐姿 / 物體）：`250_Face-Mask` / `458_HeadPose-Yaw` / `481_WHC` / `478_SC` / `471_Wholebody34` / `132_YOLOX` 等。**只研究跟 6/18 live demo 有關**，不逛整個 zoo。考量 input/output format / label map / post-process / latency / license | 選出 1-2 個進 W3 整合測試 | 不動現有模型，全壓既有 stack |
...
#### W1（5/23-5/27，5 天）— **全壓 5/27 影片功能**（2026-05-23 grill 後重排）

> **重要變更**：原 W1 規劃 D435 mount STL / STVL / Object schema 三 spike + L1 包裝層平行。5/22 grill 確認 5/27 是 hard deadline（唯一影片產出），W1 全壓 5/27 影片功能（門口監控 + 語音動作 + 安全拒絕，~400 LoC），**Nav / Object / STVL / PINTO spike 全移到 W2 之後**。詳 [5/27 spec](../../pawai-demo/2026-05-27-mid-video-spec.md) § 5。

| 日 | 工作（Roy own 功能、團隊接拍攝協調） |
|---|---|
| **5/24 Sat（code 日）** | 段 3 陌生人 FSM thin 版（~200 LoC）+ 段 4 intent_classifier 加「舉手」keyword（~20 LoC）+ 段 5 unsafe_intent=backflip 流到 SafetyLayer 路徑 + safety TTS（~60-80 LoC）+ 本機 unit test |
| **5/25 Sun（Jetson smoke 日）** | jetson-deploy 4 條 + 段 2-5 逐條 smoke + 全段串連 smoke 1 次 + 段 0 硬體素材在家拍（Go2 + 3D 件 + 降壓板）+ evening 砍項決定 |
| **5/26 Mon（排練日，不再加功能）** | 排練 1（記 bug）+ bug fix（**只修不加新功能**）+ 排練 2（錄備用素材） |
| **5/27 Tue（拍攝 + 交件）** | AM 最終排練 + demo-preflight；PM 14:00-17:00 正式錄影；Eve 剪輯 + 旁白配音 + 段 0/段 6 接合；23:59 交件 |
| **Nice-to-have（時間夠才做）** | 5/24 PM 段 2.5 遮擋 fallback occlusion event（~100 LoC）— 5/26 早上 review 5/25 smoke 結果決定是否進影片 |

**W1 砍項策略**：5/25 PM smoke 不穩時，首砍段 3 retry loop（保 1 問 1 答），次砍段 2.5 遮擋。**段 2 + 4 + 5 不可砍**（PawAI Brain 三層架構唯一可視化證據 + 呂奇傑指定鏡頭）。

#### W2-W4 — 6/18 long-term sprint（spike + 主線並行）

> **5/28 起重啟 Nav / Object / VLM / Engagement Gate / fallen state machine 主線**。spike 順序維持原版（#1 depth safety + #2 Object schema + #1d F7 + #6 PINTO shortlist 在 W2 初）。

| 週 | 主軸 | 必達 milestone |
|---|---|---|
| W2 (5/28-6/4) | **W1 因壓 5/27 沒做的 spike 補上**：#1 depth safety + #2 Object schema + #1d F7 偵錯 + #6 PINTO shortlist + L1 包裝層（skill metadata / persona 4 檔 / Mapping QA / AprilTag）+ NAV executor + named waypoint + depth safety（spike #1 PASS）+ L2-must 並行（手勢 3 補充） | spike 結果 commit + 「PawAI 去活動區」走 named waypoint + 遇障停 |
| W3 (6/5-6/11) | Engagement Gate（**studio_ptt + face + gesture，無 wake word**，詳 ADR-0003）+ VLM scene_describe + Object schema 實作（依 spike #2 完整 / thin / side-channel）+ L2-must 並行（fallen state machine **TTS silent**，詳 grill Q5）+ **W3 中 spike #5 6/18 live 連跑可行性驗** | §11 Demo B 場景劇段 1-3 E2E 首次跑通 |
| W4 (6/12-6/18) | **6/18 live demo 排練**：場景劇 8（或降規模 5-6）mini-event 連跑 5/5 + 現場 fallback 全驗 + 現場話術 audit（依 doc-and-report-guidelines.md，不是影片旁白）+ Go2 帶到學校場域實測 + 開場輔助投影片定稿（5/27 中期影片 + 未來情境 3 張靜態圖）+ stretch（spike #3 PASS 加 STVL demo 鏡頭 / 物件 YOLOv8s 視時程加分）| 6/18 live demo 連跑 5/5 通過率（不錄影片，現場展示）|
...
### A.3 v2-amended → v2-2026-05-23（三場 review synthesis + grill + ADR-0001/0002/0003）

5/23 整合三場 5 月驗收（5/18 葉承達 / 5/20 雅文 / 5/22 呂奇傑）synthesis + 兩輪 grill（5+7 個決議）+ 3 個 ADR。amendment 範圍 medium，工程能力一行沒砍，主要動作是**敘事/安全紅線/互動入口/demo 形式**四件事入 spec。

| 章 | amendment 動作 |
|---|---|
| §1 拒絕標籤 | soften「居家陪伴狗」為「**單純居家陪伴狗**」+ 加註 ADR-0002 雙層敘事；新增「**直接物理接觸長者鏡頭**」拒絕條（指向 ADR-0001） |
| §1 末 | 新增「雙層敘事 footnote」小節：平台身份 = 通用居家/機構四足互動機器人 / 2026-06 demo 場景 = 非接觸式機構巡檢助理（詳 ADR-0002）|
| §4 末 | 新增「人體安全紅線」小節（3 條落地規則，指向 ADR-0001）+「對外敘事公式」小節指向獨立 doc-and-report-guidelines.md |
| §5 紅旗 | 新增 2 條：人機接觸風險（ADR-0001 涵蓋）+ F7 bug demo blocker（W1 1d 偵錯 spike） |
| §7 Engagement Gate | **整章重寫**：wake word → studio_ptt + face + gesture 三源觸發抽象；VAD 整段拔；工程量 3d → ~1.5-2d（詳 ADR-0003）|
...
- **領域語言**入 `CONTEXT.md`（已新增 5 條：非接觸式巡檢助理 / 雙層敘事 / Engagement Gate / 產品化接線 / 安全成熟度）
