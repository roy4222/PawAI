...
bool    use_local_only   # true=不依賴 map frame（純 odom），預設 true
---
# Result
bool    success
...
| Service | Type | 行為 |
|---------|------|------|
| `/nav/pause` | `std_srvs/Trigger` | 停止 route runner 送 Nav2 goal + 記住 `current_waypoint_index`。**不假裝凍結 Nav2 active goal**（避免 BT timeout）。 |
| `/nav/resume` | `std_srvs/Trigger` | 重新發送目前 waypoint 的 NavigateToPose goal（新 goal、新 BT context）。 |
...
| # | 項目 | 何時解決 |
|:-:|------|---------|
| T1 | Driver heartbeat / connection health topic（E5 detection 改良）| 寫 implementation plan 時順便加進 driver |
