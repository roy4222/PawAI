...
- **F32** **GPU 占空比預估**：pose@10Hz × 5-8ms ≈ 5-8% engine 佔用 + object n@8Hz × 5-16ms ≈ 4-13% → **合計 <25%，遠低於 RTMPose 的 90% 前科**（`docs/architecture/perception/pose/research/2026-03-21-benchmark-decision.md:33-35` GPU 85-90%）。但 **Whisper CUDA on-demand burst 的互動必須重測**——3/21 L2 錨點：rtmpose_lw + whisper_small 同跑 = -20% FPS（`2026-03-21-benchmark-decision.md:42`；archive `benchmarks/results/archive/pose_estimation/20260321/raw.jsonl` run aaa5e2cd）。
...
- **F35** **TRT EP 整合 pattern 是現成的**：object node 的 `_init_onnx`（provider 參數 `"True"` 字串、engine cache、letterbox、`(1,300,6)` parse，`object_perception_node.py:266-303, 322-343`）可直接複製到 pose adapter，已知坑全部文件化（`docs/architecture/perception/object/CLAUDE.md` 坑 #1/#6/#9）。
...
- `docs/architecture/perception/pose/research/2026-03-21-benchmark-decision.md`（:25 YOLO11n-pose P2；:33-42 L1/L2 表；:50-60 18.5 口徑與決策）
