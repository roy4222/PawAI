...
| B5 reactive_stop | ☑ partial — 訊號通、未做 motion 驗收 | 5/11 上機：sllidar (`/scan_rplidar` 11.98 Hz, single publisher) + `base_link→laser` static TF (yaw=180°) + `reactive_stop_node` (safety_only mode, danger<0.6m, slow<1.0m, front_offset=+180°) + foxglove_bridge `ws://192.168.0.222:8765` 全跑通。danger zone 鎖死時 `/cmd_vel_obstacle` 持續發 0 @10Hz、mux 用 obstacle priority 200 蓋掉 teleop priority 100、Go2 站著不動 → 訊號鏈完整。但 motion 階段觀察到嚴重缺陷：(1) `danger<0.6m` 對 Go2 機身太近 — LiDAR 安在 base_link 前 17.5cm，Go2 機鼻在 base_link 前 ~50-60cm，LiDAR 看到 0.6m 時機鼻只剩 ~0.2m，必撞；(2) safety_only mode 在 slow/clear zone 完全不發訊號 → 0.5s 後 mux timeout 自動降級到 teleop priority 100（teleop 還在送 0.5 m/s）→ Go2 全速衝出，沒漸進減速。實測一次後 Go2 撞到 1.5m 處障礙物。後續 motion 驗收延後到修法落地後。**完整修法見 [reactive-stop-safety-fix-plan.md](./2026-05-12-reactive-stop-safety-fix-plan.md)**（subagent 5/11 night 調查、總工時 ~3.5h、5/12 早 AM 落地）：Fix 1 safety_only slow zone 也限速 0.45、Fix 2 base_link projection（機鼻投影 + danger 0.6→0.4 投影後）、Fix 3 clear 後 dwell 1s + mux timeout 0.5→1.5s、Fix 4 hysteresis 防 boundary 抖動。 |
| B6 AMCL | ☐ pass / ☐ fail | |
