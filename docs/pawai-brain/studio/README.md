...
**6 個 feature button → 點開 Sheet（從右側滑入，mobile 從底部上滑）**：
...
- **明天 Jetson session**：實機 smoke OpenRouter Gemini chain（commit `fda1b3c`）+ Go2 真 TTS hardware + 8 scene Plan A/B 連跑
- B7：60 min 連續供電壓測
