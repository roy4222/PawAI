...
使用者說「你好」
  ...
  → llm_bridge_node (Qwen2.5-7B-Instruct, max_tokens 120)
  → tts_node (Piper, +16dB gain)
  → Megaphone DataChannel (4001/4003/4002, cooldown 0.5s)
  → Go2 說話 + 揮手動作
...
**LLM**：Qwen2.5-7B-Instruct（純文字 CausalLM），max_tokens 120，prompt 限 reply 25 字
...
| `speech_processor/speech_processor/llm_bridge_node.py` | **新** — 取代 intent_tts_bridge_node，呼叫 Cloud LLM（Qwen2.5-7B-Instruct），含 force_fallback 參數 |
