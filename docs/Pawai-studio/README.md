...
| 項目 | 值 |
|------|---|
| 狀態 | **Live View 實機通過 + Mission Control UI**（4/7） |
| 版本/決策 | Next.js 16（前端）+ FastAPI + rclpy（Gateway） |
| 完成度 | Gateway ROS2 bridge ✅ + Live View 三欄影像 ✅ + 5 panel 即時數據 ✅ + Mission Control 首頁 ✅ |
| 最後驗證 | 2026-04-07（實機 Demo 通過：face greeting + thumbs_up TTS + stop 手勢） |
| 入口檔案 | `pawai-studio/gateway/studio_gateway.py`（Gateway）、`pawai-studio/frontend/`（前端） |
| 測試 | Gateway 28 tests PASS（15 原有 + 13 video bridge）、Frontend build PASS |
