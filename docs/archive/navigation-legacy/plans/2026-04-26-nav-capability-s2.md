...
**Architecture:** 新建 `nav_capability` ROS2 pkg 包裝 Nav2 NavigateToPose 為高階 API；twist_mux 升級為 emergency(255)/obstacle(200)/teleop(100)/nav2(10) + Bool lock；reactive_stop 改發 `/cmd_vel_obstacle`；Nav2 cmd_vel remap 到 `/cmd_vel_nav`。Spec: `docs/archive/2026-05-docs-reorg/superpowers-legacy/specs/2026-04-26-nav-capability-s2-design.md`。
