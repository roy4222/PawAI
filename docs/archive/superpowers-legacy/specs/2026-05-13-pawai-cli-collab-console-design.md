...
| 10 | **start.sh hardcoded `100.64.0.1`** | Studio frontend URL hardcoded; if Jetson Tailscale IP ever rotates (Tailscale generally pins IPs but not contractually) the demo breaks silently. |
...
| 3 | `pawai demo start` propagates `JETSON_TAILSCALE_IP` → studio frontend, no hardcoded fallback | `brain-studio-lane/scripts/start.sh`, `main.py:demo_start` | 1 h | Today's 5/12 night carry-over bug (start.sh hardcoded `100.64.0.1`) |
