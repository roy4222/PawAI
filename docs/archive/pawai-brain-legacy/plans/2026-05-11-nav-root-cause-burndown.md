...
| B5 reactive_stop | ☑ signal-pass / motion 待 5/12 重測 | **訊號鏈通**（5/11 上機）：sllidar `/scan_rplidar` 11.98 Hz / `base_link→laser` yaw=180° / reactive_stop_node + foxglove_bridge / danger zone `/cmd_vel_obstacle=0` 鎖 mux priority 200。**motion 階段撞 1.5m 物體**根因：(1) `danger<0.6m` 對 Go2 機鼻太近（LiDAR 在 base_link+0.175m、機鼻在 +0.40-0.60m → LiDAR 0.6m=機鼻 0.2m 必撞）；(2) safety_only 在 clear zone 沉默 → mux 0.5s timeout → teleop hot-publish 0.5 m/s 接管 → Go2 全速衝出。**修法 5/11 night code 已落地（commits `b9aac4d` driver fix → `4ec8350` B0 → `0f5a16f` 4-mode 重設計 → `d804a58` Roy 4 review fix）**：(a) driver `cmd_vel=0` 改走 `StopMove (api_id=1003)` 取代 `Move {x:0}`；(b) reactive_stop 重設計成 4-mode 狀態機（hold_brake / progressive / released / disabled / standalone）取代 binary safety_only；(c) threshold enlarge `danger 0.6→1.1m` `slow 1.0→1.7m`；(d) hysteresis clear_debounce 3→5；(e) scripts 拆 3 種啟動模式。**完整深度研究 + todo + 開放問題**見 [`docs/archive/navigation-legacy/research/2026-05-11-nav-avoidance-deep-research.md`](../../navigation/research/2026-05-11-nav-avoidance-deep-research.md)，**修法路線圖**見 [`docs/architecture/navigation/2026-05-11-architecture-deep-audit-and-fix-roadmap.md`](../../navigation/2026-05-11-architecture-deep-audit-and-fix-roadmap.md)。5/12 早 AM 上機重測（hold_brake / progressive / 釋放 3 步驟驗收）。 |
| B6 AMCL | ☐ pass / ☐ fail | |
