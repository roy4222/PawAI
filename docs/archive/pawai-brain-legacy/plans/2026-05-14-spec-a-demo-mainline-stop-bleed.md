...
**Base**：`main` @ commit `d9d4879`（`docs: 同步 5/15 學校招生 demo 強化`）。此 commit 已是明天 school demo baseline，含 `e9eaac6` / `2714599` / `b011b40` 三個 school demo commit；Spec A 全部 branch 從此往後疊。**不要從舊 spec wording 的 `a1ebdd2` 切**（已落後 4 個 commit，會製造假衝突）。
...
**背景**：plan 初稿假設前一日平行跑四個 goal 導致 branch / stash 漂移需要收拾。**2026-05-15 已重新盤點，狀態乾淨**：在 `main` @ `d9d4879`、worktree 乾淨、無 `spec-a/*` branch、無 stash、無額外 worktree。因此 D0.1 / D0.2-Step2 的「漂移回收」流程整段作廢，直接從 `d9d4879` 開乾淨的 worktree。

## D0.1：Workspace 狀態盤點 — ✅ 已完成（2026-05-15）

盤點結果：`main` @ `d9d4879`、`git status` 乾淨、`git branch --list 'spec-a/*'` 空、`git stash list` 空、`git worktree list` 僅主 workspace。**無漂移可回收，本節跳過**，直接進 D0.2 開新 worktree。

> 若日後重新進入本 plan 且 workspace 狀態不明，再依下列指令重新盤點：
> `git worktree list` / `git branch --list 'spec-a/*' --verbose` / `git stash list` / `git status --short`。
...
worktree 路徑統一用 `../elder_and_dog-spec-a-pr<N>`（與其他開發 worktree 用 `spec-a` 區隔）；branch 用簡名 `spec-a/pr<N>-<topic>`。PR1 從 `main`（@ `d9d4879`）切，其餘從前一個 sibling branch 疊。

```bash
cd <repo 根>   # 主 workspace repo 根目錄，僅用於建 worktree / sync，不跑 goal

# PR1 — 從 main @ d9d4879
git worktree add -b spec-a/pr1-static-fixes ../elder_and_dog-spec-a-pr1 main

# PR2A — 疊在 PR1 上
git worktree add ../elder_and_dog-spec-a-pr2a -b spec-a/pr2a-mechanical-guard \
  spec-a/pr1-static-fixes

# PR2B — 疊在 PR2A 上
git worktree add ../elder_and_dog-spec-a-pr2b -b spec-a/pr2b-semantic-dryrun \
  spec-a/pr2a-mechanical-guard

# PR3 — 疊在 PR2B 上
git worktree add ../elder_and_dog-spec-a-pr3 -b spec-a/pr3-behavior-gate \
  spec-a/pr2b-semantic-dryrun

git worktree list
```

PR2A/2B/3 worktree 建立時都指向 `d9d4879`（sibling branch 此刻 = main）。**每個 PR 開工前（Task X.1 Step 1）必須 `git fetch origin main && git rebase origin/main`**，把 base 對齊「前一個 PR 已 merge 進 main」的狀態 — 這是強制步驟，不是 optional，否則該 PR 會從舊 `d9d4879` 開工、漏掉前序 PR 的所有改動。

- [ ] **Step 2：~~把 stash 內改動 apply 到 worktree~~ — ✅ N/A（無 stash）**

D0.1 已確認無 stash、無漂移改動。本步驟不需執行，所有 PR 群改動從零在各自 worktree 內依後續 Task 落地。
...
- 每個 PR group goal 必須在對應 ../elder_and_dog-spec-a-pr<N> worktree 內跑。
...
- 你的工作目錄是 <repo 根的 sibling>/elder_and_dog-spec-a-pr<N>
...
- [ ] **Step 1：確認 worktree 就緒**

branch + worktree 已於 D0.2 Step 1 建好（`git worktree add -b spec-a/pr1-static-fixes ../elder_and_dog-spec-a-pr1 main`）。本步驟只需 cd 進 worktree 確認狀態，**不要再 `git checkout -b`**（branch 已存在會失敗）。

```bash
cd ../elder_and_dog-spec-a-pr1
git status        # 應顯示 working tree clean
git branch --show-current  # 應為 spec-a/pr1-static-fixes
git log --oneline -1  # 應為 d9d4879
```
...
- [ ] **Step 3：等 review 與 merge 後 — 在主 workspace 更新 main**

⚠️ PR worktree 內**不能** `git checkout main`（main 已被主 workspace checkout，git worktree 禁止同一 branch 雙 checkout，會直接報錯）。回主 workspace 更新：

```bash
cd <主 workspace repo 根>   # main 在此 worktree
git pull --ff-only
...
- [ ] **Step 1：確認 worktree 就緒 + rebase 到已 merge PR1 的 main（強制）**

branch + worktree 已於 D0.2 Step 1 建好（base = `spec-a/pr1-static-fixes` @ `d9d4879`）。cd 進 worktree，**不要 `git checkout -b`**。worktree 建立時 PR1 尚無 commits，**必須**先 rebase 到已 merge PR1 的 `origin/main`，否則會從舊 `d9d4879` 開工、漏掉整個 PR1 內容：

```bash
cd ../elder_and_dog-spec-a-pr2a
git status && git branch --show-current  # spec-a/pr2a-mechanical-guard
git fetch origin main
git rebase origin/main               # 強制：base 對齊已 merge PR1 的 main
git log --oneline -3 origin/main     # 確認含 PR1 commits
```
...
def _check_unique_publisher(topic: str, expected_node: str, check_name: str) -> CheckResult:
    # 注意：`pawai demo school ending`（b011b40）會起一個一次性 rclpy process
    # 短暫 publish /tts 後即銷毀。preflight 設計為 demo start 時跑，正常不會撞；
    # 但若有人在 ending 進行中手動跑 preflight，此 check 可能短暫看到 2 個
    # publisher。實作 / 驗收時確認：(1) ending publisher node 名可辨識並排除，
    # 或 (2) 文件明示 preflight 不可與 ending 並跑。不要為了「修」這個誤判
    # 而放寬 unique 判定。
...
- [ ] **Step 4：merge 後 — 在主 workspace 更新 main**

⚠️ 勿在 PR worktree `git checkout main`（worktree 禁止同 branch 雙 checkout）。

```bash
cd <主 workspace repo 根> && git pull --ff-only
```
...
- [ ] **Step 1：確認 worktree 就緒 + rebase 到已 merge PR2A 的 main（強制）**

branch + worktree 已於 D0.2 Step 1 建好（base = `spec-a/pr2a-mechanical-guard` @ `d9d4879`）。cd 進 worktree，**不要 `git checkout -b`**。**必須**先 rebase 到已 merge PR1+PR2A 的 `origin/main`，否則會漏掉前兩個 PR 內容：

```bash
cd ../elder_and_dog-spec-a-pr2b
git status && git branch --show-current  # spec-a/pr2b-semantic-dryrun
git fetch origin main
git rebase origin/main               # 強制：base 對齊已 merge PR1+PR2A 的 main
git log --oneline -5 origin/main     # 確認含 PR1 + PR2A commits
```
...
def maybe_inject_pose(pose: str, force: bool = False) -> bool:
    ...
    **Schema**：canonical key 為 `pose`（與 vision_perception 真實 event 對齊，
    驗於 conversation_graph_node._on_pose_detected；行號因 `e9eaac6` school
    程式碼插入已位移，用 `grep -n _on_pose_detected` 重新定位）。**不可**用 `name`，
...
- [ ] **Step 2：merge 後 — 在主 workspace 更新 main**

⚠️ 勿在 PR worktree checkout main。回主 workspace：`cd <主 workspace repo 根> && git pull --ff-only`。
...
- [ ] **Step 1：確認 worktree 就緒 + rebase 到已 merge PR2B 的 main（強制）**

branch + worktree 已於 D0.2 Step 1 建好（base = `spec-a/pr2b-semantic-dryrun` @ `d9d4879`）。cd 進 worktree，**不要 `git checkout -b`**。**必須**先 rebase 到已 merge PR1+PR2A+PR2B 的 `origin/main`，否則會漏掉前三個 PR 內容：

```bash
cd ../elder_and_dog-spec-a-pr3
git status && git branch --show-current  # spec-a/pr3-behavior-gate
git fetch origin main
git rebase origin/main               # 強制：base 對齊已 merge PR1+PR2A+PR2B 的 main
git log --oneline -8 origin/main     # 確認含前三個 PR commits
```
...
Expected：gate check 早於 confirm 處理（base @ `d9d4879` 重新確認；行號以 grep 結果為準，不依賴 plan 內寫死的 614 / 648）。
...
> ⚠️ **必須保留 5/14-15 school demo 邏輯**：base `d9d4879` 的 `e9eaac6` 在此檔加了
> `_SCHOOL_DEMO_FACTS` 與 `school_demo_request` 分支（facts injection 走字串注入，
> 不污染 persona）。Spec A 只動 `_on_pose` / `_format_current_pose` / pose prompt
> 注入，**不得移除或改動 school_demo_request 任何分支**。改完用
> `grep -n "_SCHOOL_DEMO_FACTS\|school_demo_request" conversation_graph_node.py`
> 確認 school 邏輯完整無缺。
...
# 同時確認 school demo 邏輯位置，後續改動不要碰到
grep -nE "_SCHOOL_DEMO_FACTS|school_demo_request" pawai_brain/pawai_brain/conversation_graph_node.py
```
...
def _on_pose(self, msg):
    ...
    **Event schema**：canonical key 為 `pose`（vision_perception 既有契約，
    驗於 conversation_graph_node._on_pose_detected；行號因 `e9eaac6` 已位移，
    用 grep 重新定位）。Parser **必須**讀
...
⚠️ 勿在 PR worktree checkout main。
