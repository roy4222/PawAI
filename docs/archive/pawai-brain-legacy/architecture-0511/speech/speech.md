...
> 這份是 5/12 brain-freeze-v2 的原始總覽。後續開發請搭配同資料夾拆分文件閱讀，因為本地程式碼裡有幾個細節和舊描述已不同：TTS chunk split 目前是 `60/45`，`speech_pipeline.launch.py` 不直接啟 `tts_node`，legacy bridge 若同時跑可能重複發 `/tts`。

## 0. 拆分文件索引

- [speech-runtime-flow.md](speech-runtime-flow.md)：端到端 runtime 架構、topic、launch 邊界。
- [speech-asr-vad.md](speech-asr-vad.md)：mic、echo gate、energy VAD、ASR provider fallback。
- [speech-tts-lanes-megaphone.md](speech-tts-lanes-megaphone.md)：TTS dual-route、audio tag、chunking、Megaphone 播放。
- [speech-brain-executive-integration.md](speech-brain-executive-integration.md)：speech event 如何進 Brain/Executive，以及 legacy bridge 風險。
- [speech-debug-runbook.md](speech-debug-runbook.md)：明天到學校現場排查順序。
