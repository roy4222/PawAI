...
> 這份是 5/11 freeze 的原始總覽。5/12 後續開發請搭配同資料夾拆分文件閱讀，因為 pose 的問題不只在 classifier，也在 event sparse、Brain stale window、Executive 只吃部分姿勢、demo bridge 尚未退場。

## 0. 拆分文件索引

- [pose-runtime-flow.md](pose-runtime-flow.md)：姿勢 runtime 架構、topic、backend、事件生命週期。
- [pose-classifier-rules.md](pose-classifier-rules.md)：7 種姿勢的規則、fallen gate、為什麼 sitting / crouching / bending / fallen 會互相打架。
- [pose-executive-brain-integration.md](pose-executive-brain-integration.md)：`/event/pose_detected` 如何進 Executive、Brain、demo bridge，以及哪些姿勢其實沒被正式接上。
- [pose-debug-runbook.md](pose-debug-runbook.md)：明天到學校現場排查順序、ROS 指令、常見症狀。
