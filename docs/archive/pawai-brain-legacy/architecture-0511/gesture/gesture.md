...
> 這份是 5/11 freeze 的原始總覽。5/12 後續開發請搭配同資料夾拆分文件閱讀，因為本地程式碼裡已經有幾個和舊描述不完全一致的 runtime 細節。

## 0. 拆分文件索引

- [gesture-runtime-flow.md](gesture-runtime-flow.md)：手勢 runtime 架構、topic、參數、實際 launch/config 現況。
- [gesture-pipeline-backends.md](gesture-pipeline-backends.md)：Recognizer / MediaPipe Hands / RTMPose 三條 backend、靜態分類、OK override、Wave 動態偵測。
- [gesture-executive-brain-integration.md](gesture-executive-brain-integration.md)：`/event/gesture_detected` 如何進 `interaction_executive`、Brain world state、legacy bridge，以及 wiggle 誤觸風險。
- [gesture-debug-runbook.md](gesture-debug-runbook.md)：明天到學校 debug 時的指令、檢查順序、常見症狀。
