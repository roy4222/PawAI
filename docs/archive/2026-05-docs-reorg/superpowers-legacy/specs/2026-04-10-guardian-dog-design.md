...
> **Status: superseded on 2026-04-11** by [`2026-04-11-pawai-home-interaction-design.md`](2026-04-11-pawai-home-interaction-design.md)
>
> **Note**: 本文件保留作為設計演化歷程的歷史紀錄。4/11 會議後產品定位收斂為「居家互動機器狗（兼具守護能力）」，互動 70% / 守護 30%。本文件中 Guardian Brain 三層架構、skill contract、skill queue、self_introduce、pre-action validation、Privacy-by-Design 等**技術設計 90% 被新 spec 繼承**；產品定位、Demo framing、八模組角色、命名體系則被新 spec 取代。
>
> Date: 2026-04-10
> Author: Roy
...
> **PawAI 的設計不是把 AI 裝到機器狗上，而是把感知、決策、安全與實體互動整合成一個可在家庭場景中可靠運作的 embodied guardian system。**

這句話同時可用於：專題文件摘要、答辯開場、論文 abstract、代表作自介。
...
**業界觀點（2026-04 輔大遞固科技演講）**：
- **四足機器人**：最穩定、控制難度最低、2025-2026 全球大量產
- **輪型機器人**：穩定性中等，受限於地形
- **人形機器人**：雙足行走穩定性不足，預估 2029 才量產

**NVIDIA GTC 2026（黃仁勳）**：具身智能是下一個黃金十年。Tesla 從電動車延伸到人形機器人、台灣自駕車產業大部分轉向具身智能——產業共識是實體載體才是 AI 的下一個戰場。

**對 PawAI 的意義**：選 Go2 不是因為「我們手邊有」，而是因為**在 2026 年這個時間點，四足是唯一同時成熟、可用、可量產的具身智能載體**。
...
### 家庭場景的可靠性設計（Reliability, Safety, Privacy）

家庭場景中的機器人不只要聰明，還要**可控、可停、可降級、可信任**。這不是事後補的安全章節，而是 Guardian Brain 架構的核心設計原則。
...
| **Graceful Degradation** | 四級降級：LLM → RuleBrain → fixed script → Safety-only | Layer B + Layer C 的 fallback 鏈 |
| **Privacy-by-Design** | 感知模組 local-first，不上雲 | 感知層實作 |
| **Security Mindset** | Safety event（stop / emergency / fallen）立即清空 skill queue，最後防線永遠可用 | Layer A + Skill Queue |

#### Privacy-by-Design：讓 AI 看懂行為，而不是懂個人身份

PawAI 的感知資料**預設不離開機體**。這不是事後補的隱私宣稱，而是架構選擇的自然結果：
...
**核心哲學**（借鑑 2026-04 輔大遞固科技周博士演講）：
> 「讓 AI 看懂行為，而不是懂個人身份。」

**對比反例**：某家用機器人公司因隱私爭議（家中私密畫面外流）倒閉——這是家用機器人的頭號風險，PawAI 從架構層就避開了這個陷阱。

**法規背景**（2027-2028 趨勢）：AI 使用必須搭配隱私法規，GDPR 之後各國 AI 基本法陸續出台。PawAI 的 local-first 架構剛好符合未來法規方向。

#### Security Mindset：最後防線永遠可用

Guardian Brain 任何層掛掉，Safety Layer 仍然能擋下危險動作：

```
正常：Brain → Executive → Safety → Go2
LLM 掛：Brain 降級 → Executive → Safety → Go2
Brain 掛：Executive 仍能接收 stop/emergency → Safety → Go2
所有感知掛：Safety Layer 仍能執行 stop_now / emergency_halt → Go2
遭惡意輸入（prompt injection 等）：BANNED_API gate 硬擋 → Go2 安全
```

這對應業界對「機器人資安」的關注（遞固與趨勢科技合作的機器人全方位資安防護白皮書同樣強調這一點）。Safety Layer 不只是防意外，也是防惡意的最後一道線。

#### 答辯用一句話

> 「家庭場景中的機器人不只要聰明，還要可控、可停、可降級。Guardian Brain 採 safety-first 設計，不讓 LLM 直接控制低階動作，加上感知模組全本地執行，正是考慮到家庭場景對安全與隱私的要求。」
...
答辯可講：多模態感知、embodied guardian brain、harness-oriented design、skill contract、graceful degradation、privacy-by-design、產品故事與風險控管。

### 未來研究方向（可延伸到研究所或論文）

基於現有專案基礎，以下三條延伸線與產業趨勢對齊，都能從本專案自然延伸：

| 方向 | 現有基礎 | 延伸目標 |
|------|---------|---------|
| **1. Embodied Guardian Brain / Harness Design** | 三層架構 + skill contract + pre-action validation | 擴展為通用 embodied agent 執行框架，發表 harness-oriented design 方法論 |
| **2. Visual Navigation / Short-range Approach** | D435 深度 + RPLIDAR 評估中 | 四足 odom 漂移補償 + 短距反應式避障 + 人體追蹤靠近，解決 Go2 導航痛點 |
| **3. Robot Safety / Security / Controlled Autonomy** | Safety Layer + BANNED_API + stop bypass | 對抗 prompt injection、設計可驗證的 controlled autonomy 機制、機器人資安防護 |

**為什麼是這三條**（不是多機協作 / Sim-to-Real / 機械手臂）：
- 都**直接從現有 PawAI 程式碼延伸**，不需要重新起爐灶
- 都**有產業需求**（遞固、NVIDIA、Tesla 都在做）
- 都**適合小團隊做**，不需要數萬張 GPU
- **都能回答同一個母題**：如何在有限資源下讓具身 AI 在家庭場景可靠運作
