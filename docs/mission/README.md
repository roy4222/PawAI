...
最保守，最後才碰，**不作為任何 P0 功能的前置依賴**。

**LiDAR 正式放棄**（3/26 會議決定）：Go2 內建 LiDAR 極不穩定（頻率突降至 0.2-0.7Hz），非 Go2 Pro 開放給外部開發者的功能，穩定性無法保證。即使當下穩定，Demo 當天仍可能爆掉。

下一步改用 **D435 depth camera** 做基礎反應式避障（尚未測試 D435+Go2 導航效果）。若 P0 全部穩定且有餘力，僅做極小範圍的深度避障展示。
