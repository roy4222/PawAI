...
### 三層成功標準（user 定義 + 5/3 evening 收緊 L3）
...
| **L3**（真繞開） | **Go2 越過 box（最終 base_link x > start_x + box_distance + 0.3m）+ 到達 goal（goal action `success=reached`）+ 全程無 watchdog emergency + 無人工 re-send goal** | 真自動繞開 |
| **PARTIAL**（介於 L2 與 L3） | DWB 試轉（lat_drift > 0.2m 或 az 非零）但被 reactive 接管 / 沒越過 box | 不算 L3，但證明 DWB 有繞行意圖 |

L1+L2 = 進展（明天還可以調）；L1+L2+L3 = 真 demo 可宣稱自動繞開。**PARTIAL 不能宣稱「自動繞開」**，要說「DWB 嘗試繞行但 safety 接管」。
...
| `scripts/start_nav_capability_demo_tmux_detour.sh` | **新檔**（含 depthimage_to_laserscan window，順序：tf → sllidar → d435 → depthimage_to_laserscan → robot/nav2 → reactive → navcap → foxglove → monitor）。**reactive window 必須明確帶 `-p danger_distance_m:=0.40 -p slow_distance_m:=0.80`**，否則 detour mode 距離不會生效（spec 寫了但 launch 沒傳 = 沒用） |
| `scripts/demo_waypoint_detour.sh` | **新檔**（B2 fallback，命名「**手動路點繞行示範**」）|

不改：
- 主線 `nav2_params.yaml`
- 主線 `start_nav_capability_demo_tmux.sh`
- **Runtime node source code**（nav_action_server / reactive_stop / capability_publisher / depth_safety 完全不動）
- Demo A 主線完全不破壞

**會改 launch source**（`go2_robot_sdk/launch/robot.launch.py` 加 `nav_params_file` LaunchArgument），但 default 指向現有 `nav2_params.yaml`，**不傳 arg 時行為與現在 100% 相同**。改完強制驗 Demo A launcher（見 Testing approach 段）。
...
| AMCL cov 又卡 YELLOW | Goal 1.6m 被 nav_action_server YELLOW gate 拒（cov ≤ 0.30 才允許 > 0.5m goal）| **Launch covariance_threshold 0.45 只影響 `/capability/nav_ready`，不影響 nav_action_server gate**。緩解只能：(a) 等 cov 自然收斂到 GREEN（5/3 觀察 60-90s 偶爾過）、(b) 重設 initialpose 觸發 global localization、(c) 物理推 Go2 0.3m。**如果長期 yellow，不能進 Phase 3，除非另開 demo-mode action gate 改動（今晚 out-of-scope）** |
