...
**如果報 `Could not find a connection between ...`** → TF chain 沒接。當前是用 `static_transform_publisher --x 0.30 --y 0 --z 0.20`（從 5/3 教訓記憶 + spec），但發布的是 `base_link → camera_depth_optical_frame` 還是別的 frame name 要確認。

Recovery：
```bash
ssh jetson-nano "tmux capture-pane -t nav-cap-demo:tf -p -S -30 | tail -10"
```
看當前 static_transform_publisher 命令的 from / to frame name 是什麼。如果不是 `camera_depth_optical_frame`，後面 depthimage_to_laserscan 的 `output_frame` 要對應改。
...
## Task 3: Spawn `depthimage_to_laserscan_node` 在 monitor window

**Files:** none（ros2 run，不寫成 launch script）

- [ ] **Step 1: 在 nav-cap-demo:monitor window 啟動 depthimage_to_laserscan_node**

Run:
```bash
ssh jetson-nano "tmux send-keys -t nav-cap-demo:monitor 'ros2 run depthimage_to_laserscan depthimage_to_laserscan_node \
  --ros-args \
...
注意：這 send-keys 進 monitor window 持續執行（前景 process），不會 daemon。後續驗證從另一個 SSH 跑 `ros2 topic` 命令。

- [ ] **Step 2: 確認 node alive 且沒 crash**

等 3 秒後 capture monitor window 看 log:
```bash
sleep 3 && ssh jetson-nano "tmux capture-pane -t nav-cap-demo:monitor -p -S -30 | tail -15"
```
...
Recovery: `tmux send-keys -t nav-cap-demo:monitor C-c Enter` 後修參數重試。
