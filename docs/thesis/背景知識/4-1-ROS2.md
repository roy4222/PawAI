...
- **Action(動作)**:擴充自 Service,額外支援進度回報(Feedback)與取消請求(Cancellation),適用於長時間執行且可能中途取消的任務,例如導航至目標點、執行多步驟自我介紹序列等。本專案的 PawAI Brain Skill Queue 機制即是以 Action 作為底層實作的抽象基礎。
