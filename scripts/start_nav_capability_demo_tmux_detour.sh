...
#   ROBOT_IP=192.168.123.161 MAP=/home/jetson/maps/home_living_room_v8.yaml \
#     bash scripts/start_nav_capability_demo_tmux.sh
set -euo pipefail

SESSION="${SESSION:-nav-cap-detour}"
ROBOT_IP="${ROBOT_IP:-192.168.123.161}"
MAP="${MAP:-/home/jetson/maps/home_living_room_v8.yaml}"
NAV_PARAMS="${NAV_PARAMS:-/home/jetson/elder_and_dog/go2_robot_sdk/config/nav2_params_detour.yaml}"
...
echo "[3/10] D435 RealSense (aligned depth → /camera/camera/aligned_depth_to_color/image_raw)"
...
echo "[3.5/10] D435 static TF (base_link → camera_depth_optical_frame, hack mount)"
tmux new-window -t "$SESSION" -n d435-tf
tmux send-keys -t "$SESSION:d435-tf" \
    "$ROS_SETUP && ros2 run tf2_ros static_transform_publisher --x 0.30 --y 0 --z 0.20 --roll 0 --pitch 0 --yaw 0 --frame-id base_link --child-frame-id camera_depth_optical_frame" Enter
sleep 2

echo "[3.6/10] depthimage_to_laserscan → /scan_d435 (D435 depth → fake LaserScan for costmap)"
tmux new-window -t "$SESSION" -n d435-scan
tmux send-keys -t "$SESSION:d435-scan" \
    "$ROS_SETUP && ros2 run depthimage_to_laserscan depthimage_to_laserscan_node --ros-args -r depth:=/camera/camera/aligned_depth_to_color/image_raw -r depth_camera_info:=/camera/camera/aligned_depth_to_color/camera_info -r scan:=/scan_d435 -p scan_height:=10 -p output_frame:=camera_depth_optical_frame -p range_min:=0.30 -p range_max:=3.0" Enter
sleep 3

echo "[4/10] robot.launch.py (driver + Nav2 wrapper + AMCL + mux + teleop) — using DETOUR profile"
tmux new-window -t "$SESSION" -n robot
tmux send-keys -t "$SESSION:robot" \
    "$ROS_SETUP && export ROBOT_IP=$ROBOT_IP && ros2 launch go2_robot_sdk robot.launch.py nav2:=true slam:=false map:=$MAP nav_params_file:=$NAV_PARAMS rviz2:=false foxglove:=false enable_tts:=false decode_lidar:=false" Enter
echo "  Waiting 30s for nav stack lifecycle"
sleep 30

echo "[5/10] reactive_stop_node (DETOUR mode: danger=0.40, slow=0.80)"
tmux new-window -t "$SESSION" -n reactive
tmux send-keys -t "$SESSION:reactive" \
    "$ROS_SETUP && ros2 run go2_robot_sdk reactive_stop_node --ros-args -p safety_only:=true -p front_offset_rad:=3.14159 -p danger_distance_m:=0.40 -p slow_distance_m:=0.80" Enter
sleep 3
