# 黃旭 — 手勢辨識互動設計（**從物體辨識換過來**）

> **目標**：手勢辨識是 PawAI 互動主軸的核心之一。除了基礎映射，更重要的是設計**手勢模式切換機制**——透過手勢切換機器狗的運作模式（聊天/聽故事/跳舞等）。
>
> **注意**：你之前負責物體辨識，4/11 會議後交換，現在負責手勢。物體辨識交給鄔雨彤。

---

## 你的任務

1. 用自己的鏡頭跑 MediaPipe Gesture Recognizer，測試 7 種內建手勢的辨識率
2. **設計手勢模式切換機制**（核心新任務）：決定幾個模式、每個模式對應什麼手勢、模式切換時 Go2 要說什麼做什麼
3. 填好「手勢→動作映射表」交給盧柏宇
4. Studio `/studio/gesture` 頁面
