# 鄔雨彤 — 物體辨識互動設計（**從手勢辨識換過來**）

> **目標**：決定要辨識哪些居家常見物品、偵測到後 Go2 做什麼情境反應。PawAI 對物體的關心應該是情境式的（例：人坐著+杯子=提醒喝水），不是單純辨識出來就念名字。
>
> **注意**：你之前負責手勢辨識，4/11 會議後交換，現在負責物體。手勢辨識交給黃旭。

---

## 你的任務

1. 用自己的鏡頭跑 YOLO26n（或 yolo11n 本機測試），測試居家常見物品的辨識率（不同光線、角度、高度 30cm 模擬 Go2 視角）
2. 決定情境化白名單（5-8 個 COCO class）+ 每個物品的情境 TTS 回應
3. **顏色辨識探索**（新任務，會議提到）：可否在嘈雜環境用顏色牌替代語音（舉紅色牌 = Yes）
4. 填好「物體→情境回應映射表」交給盧柏宇
5. Studio `/studio/object` 頁面
