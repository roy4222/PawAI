...
# ── 啟動 Backend ────────────────────────────────────────────────────────
echo "[3/4] 啟動 Mock Server (port 8001)..."
cd "$BACKEND_DIR"
python3 -m uvicorn mock_server:app --port 8001 --host 0.0.0.0 &
BACK_PID=$!
sleep 2
...
echo "  📡 WebSocket:    ws://localhost:8001/ws/events"
echo ""
echo "  觸發 Demo A:  curl -X POST http://localhost:8001/mock/scenario/demo_a"
echo ""
