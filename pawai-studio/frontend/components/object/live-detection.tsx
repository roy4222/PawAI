  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {inWL
          ? <span className="shrink-0 text-[9px] px-1.5 py-0.5 rounded-full bg-amber-400/10 text-amber-400 border border-amber-400/20 font-medium">白名單</span>
          : <span className="shrink-0 text-[9px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground border border-border/20">靜默</span>
        }
      </div>
      ...
      {entry && (
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[10px] text-emerald-400/80 italic flex items-start gap-1">
            <span className="shrink-0">🔊</span><span>「{entry.tts}」</span>
          </p>
        </div>
      )}
    </div>
  ...
  if (!isActive || objects.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3 text-muted-foreground">
        ...
        <p className="text-xs opacity-60">Mock Server 每 2 秒推送一次</p>
      </div>
    )
  }
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      {objects.map((obj, i) => <ObjectCard key={`${obj.class_name}-${i}`} obj={obj} idx={i} />)}
    </div>
  )
}
