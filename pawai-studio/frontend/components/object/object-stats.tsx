...
function StatBar({ label, emoji, count, maxCount, inWhitelist }: {
  label: string; emoji: string; count: number; maxCount: number; inWhitelist: boolean
}) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-1.5 w-24 shrink-0">
        ...
        <span className={cn("text-xs truncate", inWhitelist ? "text-foreground font-medium" : "text-muted-foreground")}>{label}</span>
      </div>
      <div className="flex-1 h-2 bg-border/30 rounded-full overflow-hidden">
        <div className={cn("h-full rounded-full transition-all duration-700 ease-out", inWhitelist ? "bg-amber-400/70" : "bg-muted-foreground/30")} style={{ width: `${pct}%` }} />
      </div>
      <span className={cn("text-xs tabular-nums font-mono w-6 text-right shrink-0", inWhitelist ? "text-amber-400" : "text-muted-foreground/60")}>{count}</span>
    </div>
  )
}
...
export function StatsSection({ events }: { events: PawAIEvent[] }) {
  ...
  const sorted  = [...allKeys].map((k) => [k, stats.get(k) ?? 0] as [string, number]).sort((a, b) => b[1] - a[1])
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-3 gap-2">
        {[{ label: "偵測總次數", value: total }, { label: "白名單觸發", value: wlTotal }, { label: "種類數", value: stats.size }].map(({ label, value }) => (
      ...
      <div className="flex flex-col gap-2.5">
        {sorted.map(([className, count]) => {
          ...
          return <StatBar key={className} label={getLabel(className)} emoji={entry?.emoji ?? "📦"} count={count} maxCount={maxCount} inWhitelist={isWhitelisted(className)} />
        })}
      </div>
