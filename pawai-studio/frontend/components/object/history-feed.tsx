...
// history-feed.tsx — 📋 偵測記錄 Feed（PR #40 port）
      ...
      <div className="flex flex-wrap gap-1.5 flex-1">
        {wl.map((obj, i) => {
          ...
          return (
            <span
              key={`wl-${i}`}
              className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-amber-400/10 border border-amber-400/20 text-amber-300"
            >
        ...
        {mute.map((obj, i) => (
          <span
            key={`mute-${i}`}
            className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full bg-muted/50 border border-border/20 text-muted-foreground"
          >
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {objectEvents.map((evt, i) => (
        <FeedRow key={`${evt.id}-${i}`} event={evt} />
      ))}
    </div>
  )
}
