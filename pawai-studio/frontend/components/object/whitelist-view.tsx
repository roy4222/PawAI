      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="rounded-xl border border-border/30 overflow-hidden">
          ...
          {OBJECT_WHITELIST.map((entry) => (
            <div key={entry.className} className="grid grid-cols-[36px_52px_60px_1fr_1fr] items-start border-b border-border/10 last:border-0 hover:bg-amber-400/5 transition-colors">
              ...
              <div className={cn("px-2.5 py-2.5 text-[10px] leading-relaxed",
                entry.testNote.startsWith("✅") ? "text-emerald-400/70" :
                entry.testNote.startsWith("⚠️") ? "text-amber-400/70" : "text-muted-foreground/60"
              )}>{entry.testNote}</div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="rounded-xl border border-border/20 overflow-hidden opacity-70">
          ...
          {EXCLUDED_ITEMS.map((item) => (
            <div key={item.className} className="grid grid-cols-[36px_52px_60px_1fr] items-center border-b border-border/10 last:border-0">
