...
PawAI 專案的領域語言與真相來源索引。本檔只當入口，細節以程式碼、contracts 與 docs 為準。

## 真相來源

- 工作慣例與環境：`CLAUDE.md`
- 專案定位、Demo 目標、北極星：`docs/mission/README.md`
- 系統架構：`docs/architecture/README.md`
- ROS2 介面契約：`docs/contracts/interaction_contract.md`
- 操作與救火 SOP：`docs/runbook/README.md`
- 架構決策：`docs/adr/README.md`

## 核心領域語言

- **PawAI Brain**：三層決策引擎（Safety → Policy → Expression），負責提出意圖與回應策略。
- **Interaction Executive**：Layer 3 中控，負責安全檢查、狀態機與所有 robot-body action 的唯一出口。
- **Skill**：Brain 提案、Executive 執行的高階動作單位（如 greet_known_person、wave_hello）。
- **Event vs State**：event = 觸發式 JSON 訊息；state = 持續式 JSON 狀態。
- **Go2 Megaphone**：Go2 內建喇叭播放走 WebRTC DataChannel api_id 4001/4003/4002。
- **Lane**：開發切換上下文單位，例如 brain / perception / speech / nav / studio。
...
不熟術語時先查這份、`CLAUDE.md` 與 `docs/README.md`，再決定要不要 grill。