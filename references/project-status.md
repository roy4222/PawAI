...
## 6/15 續4（夜）：CLI 驗證綠 + Act1 停障餘裕加大（Roy 收工前）

**主軸**：Roy 把 Go2+Jetson 帶去學校（隔天測），收工前要：① 驗證 PawAI CLI（人臉註冊/列出/刪除）讓明天可用 ② 確認 Act1「為何會撞」有解。**Jetson 離線＝零實機驗，只能做軟體面 + 文件。**

### CLI 驗證（綠）
- **CLI 套件 254 passed**：修掉一條紅燈 `test_v2_demo_start_kwargs_match_legacy_signature` — legacy `pawai demo start` 早先加了 `--with-lidar`，但沒同步到 `pawai2`(v2 Typer skeleton) → parity 測試失敗；已補 `--with-lidar` 轉發。`pawai2` 仍非 demo 主線（走舊 `pawai`）。
- **人臉指令面齊全**：`pawai face list/enroll/delete/rebuild/test`（全中文 help），Windows 原生 PowerShell 已放行（純 SSH）。**Jetson 離線**，live `face list` 連線逾時（CLI 優雅報錯、不卡死）→ 真機 SSH 執行待 Roy 明天開機驗。

### Act1「會走但撞」根因 + 修正
- **不是謎團**：先前「狗不動」兩 bug（`set -u` 殺腳本、`/cmd_vel` 雙 publisher 交錯）已修＝狗會走。Roy 後測「有前進但撞」＝**停障餘裕太緊**：Go2 sport 速度地板 0.5（無法更慢）跑 0.6m/s + WebRTC/sport 煞停延遲 + LiDAR 裝機鼻後 ~0.32m → `danger=1.1` 剛好在邊緣（6/15 一次停障礙前 30cm、一次撞上）。
- **修正（保守方向、env 可還原）**：`start_reactive_forward_demo.sh` `danger 1.1→1.5`/`slow 1.3→1.8`；`act1_forward.sh` 手動前進預設 `2.0s→1.0s`(≈0.6m)；`act1_voice_trigger.py` pre-check `DANGER_M 1.1→1.5`；Act1 runbook 同步。`ACT1_DANGER_M` 可覆蓋（1.8 更早停 / 1.1 回舊）。
- **誠實邊界**：軟體只**降低**撞機率、**不保證 0 撞** — 0.5m/s 地板拿不掉 + `±18°` 窄錐側前盲區（加大 danger 修不掉）。明天保證不撞靠 **正前淨空跑道 + 手持 e-stop + 短距 + 不穩立刻 fallback**。

### 明天用前必做（Roy）
1. `pawai jetson deploy`（Act1 安全修正在 script 層，不 deploy = Jetson 仍跑舊 danger 1.1）。
2. `pawai demo start --with-lidar` → 快測 S2-S5 → Act1 只測**空曠 0.5m**（e-stop 在手）→ 過了就收，不硬測障礙停。
- Windows/Mac CLI 實機、Jetson live face/Act1 motion 仍待 Roy。

---

## 6/15 續3（夜，Roy AFK）：Cloud 三件事收斂 — Act1 根因+修回 standalone / offline G2 / Mac CLI

**主軸**：Roy 出門前收斂成剩三件（Act1 語音/文字觸發 motion、offline fallback、Mac CLI），交 Cloud 在他回家前做完軟體面 + 找出問題。打法＝3 個並行唯讀調查 agent（motion mux 根因 / offline / Mac CLI）→ 我據結果開發。**硬前提：Roy 不在＝零 motion 實測**，Act1 只做到「code 改好+邏輯驗過+標 NEEDS_ROY_ESTOP_TEST」，真機留給 Roy e-stop。

### Task 1 — Act1 voice/text→motion：撞車根因查清 + 修回 standalone
...
- **對抗複查強化（第一版有洞、已修）**：唯讀對抗審查抓到 2 blocker：**A-1** `ACT1_KILL_COMPETITORS` 預設 0 = fail-open（brain mux 也發 /cmd_vel → 雙 publisher 打架、danger-stop 被打斷）→ 改**預設 1** + 啟動後驗 `/cmd_vel` publisher==1 否則**拒絕進 motion**；**A-2** voice `subprocess timeout=15` 可能 SIGKILL bash、留 `enable=true` 狗續走 + `ros2 topic echo --once` 在忙碌/死掉的 reactive 上會 hang → `act1_forward.sh` 重設計成 **`trap EXIT/TERM/INT` 保證 force_stop** + 定時放行（`FORWARD_S` 2.0s）、砍掉輪詢。**A-3**（±18° 窄錐側前盲區、違「窄錐≤0.2」鐵律）＝Roy 拍板參數不擅改、runbook §6 補**紅字盲區**。offline 對抗審＝**clean**；Mac＝文案修（C-1「已驗證」→「靜態分析未實機」、C-2 補「Mac 別觸發 Act1」）。**三輪複查後判定兩機制 close、無 hard blocker、可交 Roy e-stop 驗**：round2 抓 A-1 雙 publisher fail-open + A-2 SIGKILL bypass bash trap → 改 **Python finally 保證停車** + **fail-closed 輪詢 gate**；round3 抓 #3「停車真正靠 enable=false 非那串 0、且失敗被靜默吞」→ 改 retry+log、+ #1 daemon Ctrl-C 競態（記殘留、e-stop 兜底）。停車真正靠 `enable=false`（reactive 停發 0.6），那串 0 是 reactive 閉嘴後才乾淨觸發 StopMove。
- **治本（post-6/18）**：driver 層 cmd_vel 供給 watchdog（>Ns 無 cmd_vel 且上一個非零 Move→自動 StopMove），把 Go2 最後-Move 滑行在 actuation 層收口。本次不做（改 actuation、需真機校）。

### Task 2 — offline / 斷網 fallback：骨架已通，修 G2 footgun
- **offline_mode 端到端已接通**（與舊註解「假設契約」相反）：ROS param + `/brain/offline_mode` topic + Studio toggle 三入口共用 `_set_offline_mode`，預設 off = byte-identical。
- **S5 安全 offline 完全免疫**：`safety_layer.py` rule-first 三層（關鍵字偵測→say_canned + request_backflip→`validate()` banned_api:1301 原子 reject→`BLOCKED_BY_SAFETY`、Go2 不發 motion），在 phase gate/LLM/offline 之前，LLM 無法 override。brain 不直呼 LLM。
- **demo 亮點句本就 offline-immune**：greet「Roy 歡迎回來」/ cup 補水句 / gesture WeGo 都是 perception **自發、rule-based**，不靠 LLM → offline 照常。offline canned 表只是 free-text chat 的保底。
- **G2 修法（唯一實戰風險）**：offline + `demo_phase=all`（**預設值**）時 canned fallback 會吐「我聽不太懂」（同源「手動切 phase footgun」、6/15 已咬過）→ 加 `OFFLINE_GENERIC_FALLBACK="嗯，我在聽，我們繼續吧。"`（**只動 offline 路徑**；online-timeout 路徑保持 byte-identical「我聽不太懂」/ T8 不破）。
- **G4 已安全**：offline=false 斷網時 `chat_wait_ms=2000` 保證 2s 回 canned、遲到 candidate 被 drop（`buffered is None`）→ 不 dead-air、不雙講。**鐵則：別把 chat_wait_ms 改回 20000**。
- **驗證**：interaction_executive **446 tests 綠**（含 2 個新 G2 test）。檔案：`brain_node.py`、`test_brain_node.py`。

### Task 3 — Mac CLI：macOS 已一級支援、零 code 改
- **macOS 明確放行**：`platform.py:37-39` Darwin→`supported=True`（不會 exit 10）、有 regression 測試（`test_platform.py`）；`/mnt/c` 檢查只對 wsl2 生效、不誤擋 Mac。
- **Mac = SSH 跳板**：8 指令 7 個純 SSH（ROS2 全在 Jetson）；唯一例外 `demo start` 會在 Mac 跑 Studio frontend（需 Node.js）。**face enroll 在 Jetson headless 跑、訂 Jetson 相機、不需 Mac 鏡頭**。
- **lock 紅項關掉**：`lock.py` 所有 `flock`+lock 讀寫全走 `shell.run_remote()`（Jetson 上跑），`/tmp/pawai-demo-lock.flock` 是 Jetson 路徑 → **Mac 不需本機 flock(1)**。**零 code 需要改**。
- **CLI 主線=legacy `pawai`**（`pawai2` 還沒接 `--with-lidar`，6/18 別用）。
- **deliverable**：`docs/pawai_cli/2026-06-15-mac-field-operation-runbook.md`（安裝+操作+fallback）。**實機綠燈待 Roy 在 Mac 跑**：① `pawai demo start` lock acquire ② `next dev` 起得來+localhost:3000 ③ doctor 實際輸出 ④ /bin/bash 3.2。

### 待 Roy（回家）
- **Act1**：確認 Go2 撞後無損 → 手持 e-stop 照 runbook 驗 Step1/2/3（不穩立刻 fallback、別動 S2-S5）。
- **offline**：Jetson dry-run（`offline_mode true`+切 phase+灌文字看秒回 canned；翻跟斗→S5 拒）。canned 15 句 + `OFFLINE_GENERIC_FALLBACK` 簽核。
- **Mac**：實體 Mac 跑 runbook，貼回 doctor / demo start / demo stop 輸出。
...
- **demo_forward profile**（Roy 拍板，室內短距直行 + 正前停障；**非完整避障導航**）：`front_arc_deg=18`（只看正前、忽略側牆）/ `danger=1.1`（**6/15 續4 撞車修正 →1.5**）/ `slow_distance=1.3`（**→1.8**）/ **`slow_speed=0.6`=跳過 slow 區**（避開 Go2 MIN_X 0.5 卡住）/ `normal_speed=0.6`。
