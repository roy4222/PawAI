...
#
# Original cmd_vel chain (Humble navigation_launch.py):
    ...
    load_nodes = GroupAction(
        ...
        actions=[
            Node(
                ...
                # nav_capability remap (1/4): controller publishes intermediate topic,
                # not cmd_vel_nav (which is now the final output of velocity_smoother).
            ...
            Node(
                ...
                parameters=[configured_params],
                arguments=['--ros-args', '--log-level', log_level],
                # nav_capability remap (2/4): smoother reads from controller's intermediate
                # cmd_vel_unsmoothed and publishes the final smoothed output to cmd_vel_nav
                # (replacing the upstream behavior that pubbed to /cmd_vel directly).
                remappings=remappings +
                        [('cmd_vel', 'cmd_vel_unsmoothed'),
    ...
    load_composable_nodes = LoadComposableNodes(
        ...
        composable_node_descriptions=[
            ComposableNode(
                ...
                parameters=[configured_params],
                # nav_capability remap (3/4): same as remap 1/4 but composable variant.
                remappings=remappings + [('cmd_vel', 'cmd_vel_unsmoothed')]),
            ComposableNode(
                package='nav2_smoother',
            ...
            ComposableNode(
                ...
                name='velocity_smoother',
                parameters=[configured_params],
                # nav_capability remap (4/4): same as remap 2/4 but composable variant.
                remappings=remappings +
                           [('cmd_vel', 'cmd_vel_unsmoothed'),
                            ('cmd_vel_smoothed', 'cmd_vel_nav')]),
            ComposableNode(
