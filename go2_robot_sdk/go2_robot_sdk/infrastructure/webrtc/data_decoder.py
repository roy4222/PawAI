        ...
        if enable_lidar_decoding:
            try:
                if OriginalLidarDecoder:
                    ...
                    decoder_name = (
                        "LidarDecoderLz4" if USING_LZ4_DECODER else "LidarDecoder"
                    )
    ...
    def decode_array_buffer(self, buffer: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if len(buffer) < 4 + json_length:
                logger.error(
                    f"Buffer too short for JSON segment. Expected {4 + json_length}, got {len(buffer)}"
                )
                return None
...
def deal_array_buffer(
    buffer: bytes, perform_decode: bool = True
