    ...
    def publish_joint_state(self, robot_data: RobotData) -> None:
        ...
        try:
            ...
            joint_state.position = [
                motor_state[3]["q"],
                motor_state[4]["q"],
                motor_state[5]["q"],  # FL leg
                motor_state[0]["q"],
                motor_state[1]["q"],
                motor_state[2]["q"],  # FR leg
                motor_state[9]["q"],
                motor_state[10]["q"],
                motor_state[11]["q"],  # RL leg
                motor_state[6]["q"],
                motor_state[7]["q"],
                motor_state[8]["q"],  # RR leg
            ]
    ...
    def publish_camera_data(self, robot_data: RobotData) -> None:
        ...
        try:
            ...
            if self.config.conn_mode == "single":
                ...
                frame_id = f"robot{robot_data.robot_id}/front_camera"

            camera_info.header.frame_id = frame_id

            if self.config.publish_raw_image:
                ros_image = self.bridge.cv2_to_imgmsg(
                    camera.image, encoding=camera.encoding
                )
                ros_image.header.stamp = stamp
                ros_image.header.frame_id = frame_id
                self.publishers["camera"][robot_idx].publish(ros_image)

            if self.config.publish_compressed_image:
                encode_params = [
                    int(cv2.IMWRITE_JPEG_QUALITY),
                    int(self.config.jpeg_quality),
                ]
                success, encoded = cv2.imencode(".jpg", camera.image, encode_params)
                if not success:
                    logger.error("Failed to encode compressed image")
                else:
                    ...
                    self.publishers["camera_compressed"][robot_idx].publish(
                        compressed_msg
                    )

            if self.config.publish_raw_image or self.config.publish_compressed_image:
