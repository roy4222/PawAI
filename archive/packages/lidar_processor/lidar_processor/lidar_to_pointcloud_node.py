...
class LidarToPointCloudNode(Node):
    ...
    def __init__(self):
        ...
        if self.config.save_map:
            self.save_timer = self.create_timer(
                self.config.save_interval, self._save_map_callback
            )
    ...
    def _load_configuration(self) -> LidarConfig:
        ...
        robot_ip_lst = (
            self.get_parameter("robot_ip_lst").get_parameter_value().string_array_value
        )
    ...
    def _setup_subscriptions(self) -> None:
        ...
        if len(self.config.robot_ip_list) == 1:
            ...
            subscription = self.create_subscription(
                PointCloud2, "/point_cloud2", self._lidar_callback, self.qos_profile
            )
    ...
    def _setup_publishers(self) -> None:
        ...
        self.pointcloud_pub = self.create_publisher(
            PointCloud2, "/pointcloud/aggregated", self.qos_profile
        )
