...
**WebRTC 音訊播放速查**：
- ~~`4001` 開始播放 → `4003` 音訊資料塊 → `4002` 停止播放~~（Go2 v1.1.7+ 已失效）
- **現行方式**：WebRTC audio track（sendonly），透過 `/tts_audio_raw` topic 送 WAV bytes
...
### Go2 DataChannel Megaphone API 失效（v1.1.7+）

api_id 4001/4003/4002（Megaphone 模式）在 Go2 韌體 v1.1.7 上完全被忽略。運動指令（1004/1005）正常。已改用 WebRTC audio track（sendonly）播放 TTS。詳見語音模組 README。
